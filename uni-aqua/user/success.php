...
if (array_key_exists('paymentId', $_GET) && array_key_exists('PayerID', $_GET)) {
    ...
    if ($response->isSuccessful()) {
        ...
        $sql = sprintf("INSERT INTO payment(payment_id, payer_id, payer_email, amount, currency, payment_status, date) VALUES('%s', '%s', '%s', '%s', '%s', '%s', CURRENT_TIMESTAMP)", $payment_id, $payer_id, $payer_email, $amount, $currency, $payment_status);
        $db->query($sql);
